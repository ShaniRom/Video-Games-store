async function loadErp(){
  ...
  //gets the games from the server then renders it. the handles here just invokes it the load function
  // couldve just written load game in the html instead of handle and then invoke it from outside
}
...
async function handleAddGame(ev){
    ...
    // the name before value has to equal the name we gave it in the input
...
async function handleDeleteGame(ev){
    ...
    // can do delete instead of post but have to write it{data:{deletedGame}}- andrew showed so
